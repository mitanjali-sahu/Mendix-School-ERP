...
public class OpenIDUtils {
    ...
    private static final Pattern UUID_REGEX = Pattern.compile("(\\p{XDigit}{8}-\\p{XDigit}{4}-\\p{XDigit}{4}-\\p{XDigit}{4}-\\p{XDigit}{12})");
    ...
    public static String extractUUID(String openID) {
        if (openID != null) {
            final Matcher m = UUID_REGEX.matcher(openID);
            if (m.find()) {
                return m.group(1);
            }
        }
